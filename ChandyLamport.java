...
class NodepReceive extends Thread{
	...
	//Node nodep = new Node();
	public void run()import java.io.DataInputStream;
import java.io.DataOutputStream;
import java.io.IOException;
import java.net.InetAddress;
import java.net.ServerSocket;
...
class Node {
	...
	public void sendMessage(String str){
		try{
			...
			Socket socket = serversocket.accept(); //listen port 40000 for node q
			//String str = "1";
			//System.out.println("connect success ! port:"+port1);
			DataOutputStream out = new DataOutputStream(socket.getOutputStream());
			out.writeUTF(str);
			socket.close();
			serversocket.close();
	...
	public void receiveMessage(){
		try{
			Socket socket = new Socket(InetAddress.getLocalHost(),port2);
			//ServerSocket serversocket = new ServerSocket(port2);
			//Socket socket = serversocket.accept(); //listen port 40000 for node q
			//System.out.println("listening... port:"+port2);
			DataInputStream in = new DataInputStream(socket.getInputStream());
			String message = in.readUTF();
			if(message.equalsIgnoreCase("marker")){
				//do chandy lamport algorithm
...
class NodepSend extends Thread{
	...
	public void run(){
		while(true){
		//System.out.println("Node P is sending a message!");
		nodep.sendMessage("1");
		//if(nodep.localState == 101){
		if(nodep.localState == 9){//make it smaller
			nodep.sendMessage("marker");
		}
		}
...
class NodepReceive extends Thread{
	...
	public void run(){
		while(true){
		//System.out.println("Node P received a message!");
...
class NodeqSend extends Thread{
	...
	public void run(){
		while(true){
		//System.out.println("Node Q is sending a message!");
		nodeq.sendMessage("1");	
		}
	}
}
class NodeqReceive extends Thread{
	...
	public void run(){
		while(true){
		//System.out.println("Node Q received a message!");
